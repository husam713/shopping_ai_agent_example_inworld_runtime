...
export function Chat(props: ChatProps) {
  const { chatHistory, connection, player } = props;
  ...
  const stopRecording = useCallback(() => {
    console.log('[Chat] User stopped recording - resuming after interruption');
  ...
  const startRecording = useCallback(async () => {
    try {
      console.log('[Chat] User started recording - interrupting audio');
      player.interrupt();
      setIsRecording(true);
  ...
  const handleSend = useCallback(() => {
    if (text) {
      console.log('[Chat] User sending text - interrupting current audio');
      player.interrupt();
      connection.send(JSON.stringify({ type: 'text', text }));

      setText('');
    }
  }, [connection, text, player]);
  ...
  return (
    ...
    >
      <History history={chatHistory} connection={connection} />
      <ActionsStyled>
        <TextField
          variant="standard"
          fullWidth
