...
export class InworldApp {
  ...
  async initialize() {
    ...
    this.toolChoice = env.toolChoice ?? 'auto';
  ...
  async load(req: any, res: any) {
    ...
    console.log('📋 Agent configuration received:', {
      name: agent.name,
      hasDescription: !!agent.description,
      hasMotivation: !!agent.motivation,
  ...
  private createSystemMessage(agent: any) {
    let systemMessage = `You are: "${agent.name}". Your persona is: "${agent.description}". Your motivation is: "${agent.motivation}".`;
