...
export class MessageHandler {
  ...
  async handleMessage(data: RawData, key: string) {
    ...
    switch (message.type) {
      case EVENT_TYPE.TEXT:
        // Interrupt any current execution for text input
        this.interruptCurrentExecution(key, interactionId);
            ...
            if (vadResult === -1) {
              ...
              if (this.pauseDuration > this.PAUSE_DURATION_THRESHOLD_MS) {
                ...
                if (speechDuration > this.MIN_SPEECH_DURATION_MS) {
                  ...
                  // Interrupt any current execution for speech input
                  this.interruptCurrentExecution(key, interactionId);
  ...
  private interruptCurrentExecution(key: string, newInteractionId: string) {
    if (this.currentExecution) {
      console.log(`[MessageHandler] Interrupting current execution for new interaction: ${newInteractionId}`);
      
      // Send interrupt signal to client
      this.send(EventFactory.interrupt(newInteractionId));
      
      // Clean up current execution
      try {
        if (this.currentExecution.outputStream && this.currentExecution.graphWrapper) {
          this.currentExecution.graphWrapper.graph.closeExecution(this.currentExecution.outputStream);
        }
      } catch (error) {
        console.warn('[MessageHandler] Error during execution cleanup:', error);
      }
  ...
  private async executeGraph({
    ...
    // Store current execution for potential interruption
    ...
    try {
      ...
      console.error('[MessageHandler] Error during graph execution:', error);
    } finally {
      // Clean up current execution
      if (this.currentExecution && this.currentExecution.interactionId === interactionId) {
